...
This is a **project-local skill** installed at `.claude/skills/ui-ux-pro-max/` in the
`96orge/finance` repo. Invoke the script by its repo-relative path from the project root
...
If `python` is not found, try `python3`, then `py -3`. Requires Python 3.x, no external
dependencies.

> **Setup note for this machine:** Python 3 is **not currently installed** here (only the
> Windows Store stub alias, which does nothing). Until a real Python 3 is on PATH, the
> `search.py` tool cannot run — fall back to `references/quick-reference.md` and
> `references/pro-rules.md` (both are plain Markdown and always usable) and say so. To
> enable the search tool, the user can run `winget install Python.Python.3.12` or install
> from python.org, then re-open the shell. Per this skill's own policy, an AI agent must
> not install Python itself — ask the user.
